...
def get_scheduler(cfg, optimizer):
	...
	if getattr(cfg.trainer, 'epoch_full', None):
		cfg.trainer.iter_full = cfg.train_data.train_size * cfg.trainer.epoch_full
	else:
		cfg.trainer.epoch_full = cfg.trainer.iter_full / cfg.train_data.train_size
	if kwargs['warmup_iters'] > -1:
		...
		if not use_iters:
			t_initial, warmup_t, decay_t, patience_t = [t / cfg.train_data.train_size for t in [t_initial, warmup_t, decay_t, patience_t]]
		...
		cfg.trainer.iter_full = cfg.trainer.epoch_full * cfg.train_data.train_size
	else:
